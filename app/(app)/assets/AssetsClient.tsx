...
export default function AssetsClient({ assets, people, types, statuses }: { assets: Asset[]; people: Person[]; types: string[]; statuses: string[] }) {
  ...
  const [f, setF] = useState({ name: "", type: types[0], serial: "", provider: "", assignedTo: "", status: "in-use", cost: "", purchaseDate: "", notes: "" });
  ...
  const totalCost = useMemo(() => view.reduce((t, a) => t + (Number(a.cost) || 0), 0), [view]);
  ...
  async function add() {
    ...
    if (r.ok) {
      ...
      setF({ name: "", type: types[0], serial: "", provider: "", assignedTo: "", status: "in-use", cost: "", purchaseDate: "", notes: "" });
      setOpen(false);
      router.refresh();
    ...
    <>
      <div className="between" style={{ marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
        ...
        <div className="row" style={{ gap: 8 }}>
          <a className="btn btn-ghost" href="/api/assets/export" download>⬇ Export CSV</a>
      ...
      {open && (
        <div className="card pad" style={{ marginBottom: 16 }}>
          <div className="grid g-3" style={{ gap: 10, marginBottom: 10 }}>
            ...
            <div><label className="lbl">Cost ₹ (optional)</label><input className="inp" inputMode="decimal" placeholder="0.00" value={f.cost} onChange={(e) => setF({ ...f, cost: e.target.value.replace(/[^\d.]/g, "") })} /></div>
      ...
      <div className="row" style={{ gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
        ...
        {totalCost > 0 && <span className="tiny faint" style={{ marginLeft: "auto", alignSelf: "center" }}>Total cost (shown): <b>₹{totalCost.toFixed(2)}</b></span>}
      </div>
      ...
      <div className="card">
        <div className="tbl-wrap">
          <table className="tbl">
            <thead>
              <tr><th>Asset</th><th>Serial</th><th>Provider</th><th>Assigned</th><th>Status</th><th style={{ textAlign: "right" }}>Cost</th><th style={{ textAlign: "right" }}></th></tr>
            </thead>
            <tbody>
              {view.map((a) => (
                <tr key={a.id}>
                  ...
                  <td className="num" style={{ textAlign: "right", whiteSpace: "nowrap" }}>{a.cost ? `₹${Number(a.cost).toFixed(2)}` : "—"}</td>
                  <td style={{ textAlign: "right" }}>
                    <button className="chip" onClick={() => del(a.id)} disabled={busy === a.id} style={{ padding: "4px 8px", fontSize: 11 }}>✕</button>
                  </td>
                </tr>
              ))}
              {view.length === 0 && <tr><td colSpan={7} className="tiny faint" style={{ textAlign: "center", padding: 22 }}>No assets yet — add your first above.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
